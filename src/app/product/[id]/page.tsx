...
import { PRODUCTS } from '../../../constants'; // Pastikan path relatifnya benar
...
export default function ProductDetail() {
  ...
  return (
    <div className="bg-madinah-light min-h-screen pb-20 font-sans text-black">
      {/* Header dengan area klik tombol kembali yang luas */}
      ...
      <div className="max-w-4xl mx-auto mt-6 px-4">
        ...
        <div className="bg-white rounded-3xl p-6 mt-6 border-2 border-gray-100 shadow-sm">
          ...
          <div className="flex items-center text-gray-600 text-sm font-bold mb-6">
            <MapPin size={18} className="mr-2" /> {product.location}
          </div>

          {/* Deskripsi */}
          <div className="border-t pt-6">
            <h3 className="font-black uppercase text-sm mb-4 tracking-widest text-gray-400">Deskripsi:</h3>
            <div className="whitespace-pre-line text-lg leading-relaxed font-medium">
              {product.description}
            </div>
          </div>

          {/* --- BAGIAN VIDEO (Muncul jika ada) --- */}
          {product.videos && product.videos.length > 0 && (
            <div className="mt-8 border-t pt-6">
              <h3 className="font-black uppercase text-sm mb-4 tracking-widest text-gray-400">Video Kondisi:</h3>
              <div className="grid grid-cols-1 gap-6">
                {product.videos.map((vid, index) => (
                  <video 
                    key={index}
                    controls 
                    className="w-full rounded-3xl border-2 border-black shadow-md bg-black"
                  >
          ...
          <div className="mt-10">
            <a 
              href={`https://wa.me/${product.whatsapp || '6285256162879'}?text=Assalamu'alaikum, tertarik dengan unit ${product.name}`}
              target="_blank"
              className="flex items-center justify-center gap-3 bg-black text-white py-5 rounded-2xl font-black text-lg hover:bg-gray-800 transition shadow-xl active:scale-95"
            >
              <MessageCircle size={24} /> HUBUNGI PENJUAL
            </a>
          </div>
        </div>
      </div>
